...
export class FsFilterComponent implements OnInit, OnDestroy {
  ...
  constructor(private FsUtil: FsUtil, private FsArray: FsArray,
    private FsStore: FsStore, private router: Router, private route: ActivatedRoute) { }
  ...
  ngOnInit() {
    ...
    if (this.filter.fsConfig.persist) {
      ...
      if (!this.filter.fsConfig.persist.name) {
        this.filter.fsConfig.persist.name = this.router.url;
      }
